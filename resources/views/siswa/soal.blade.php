    ...
    <body>
        ...
        <script>
            function simpan_jawaban(id){
                ...
                $.ajax({
                    url: "{{ url('siswa/ujian/simpan_jawaban') }}",
                    type: "POST",
                    data: {
                        '_token' : "{{csrf_token()}}",
                        'id' : id_jawaban_ujian,
                    ...
                    success: function(response){
                        //window.location.href="{{url('siswa/ujian_page')}}"+"/"+id;
                        console.log('success');
                    },
                })
            }
            ...
            function reset(){
                ...
                $.ajax({
                    url: "{{ url('siswa/ujian/reset_jawaban') }}",
                    type: "POST",
                    data: {
                        '_token' : "{{csrf_token()}}",
                        'id' : id_jawaban_ujian,
            ...
            function selesai(){
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        var id_ujian = {!! json_encode($soal->id_ujian) !!};
                        window.location.href = `{{url('siswa/hasil_ujian')}}`+`/`+id_ujian;
                        //reset storage timer
                        //resetTimer();
                    }
                });
            ...
            id_jawaban_siswa = {!! json_encode($soal->id) !!};

            $.ajax({
                url: "{{ url('soal_detail_baru') }}",
            ...
            let durasi_max_minutes = {!! json_encode($soal->durasi_max) !!};
            let durasi_min_minutes = {!! json_encode($soal->durasi_min) !!};
        ...
        function resetUjian(){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    var id_ujian = {!! json_encode($soal->id_ujian) !!};
                    window.location.href = `{{url('siswa/reset_ujian')}}`+`/`+id_ujian;
                    //reset storage timer
                    //resetTimer();
                }
            });
